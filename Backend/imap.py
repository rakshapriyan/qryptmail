...
def get_emails(user: str, password: str) -> List[EmailModel]:
    ...
    for idx, e_id in enumerate(email_ids):
        ...
        email_excerpt = body[:50] + '...'  # Preview of the email content
...
@app.post("/emails", response_model=List[EmailModel])
def read_emails(email_request: EmailRequest = Body(...)):
    try:
        return get_emails(email_request.email, email_request.password)
